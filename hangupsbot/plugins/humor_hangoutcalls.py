        ...
        if lastcall:
            ...
            if bot.conversations.catalog[event.conv_id]["type"] == "ONE_TO_ONE":
                ...
                yield from bot.coro_send_message(event.conv_id,
                                                 _(
                                                     "<b>It's been {} since the last call. Lonely? I can't reply you as I don't have speech synthesis (or speech recognition either!)</b>").format(
                                                     humantime))
            else:
                """subsequent calls for a GROUP"""
                yield from bot.coro_send_message(event.conv_id,
                                                 _(
                                                     "<b>It's been {} since the last call. The last caller was <i>{}</i>.</b>").format(
                                                     humantime, lastcaller))
        else:

...
def _watch_twitter_link(bot, event, command):
    ...
    try:
        ...
        timeago = prettydate(datetime.datetime.now(tz=datetime.timezone.utc) - datetime.datetime.strptime(time,
                                                                                                          '%a %b %d %H:%M:%S %z %Y'))
        logger.info(timeago)
        username = tweet['user']['name']
        twhandle = tweet['user']['screen_name']
        userurl = "https://twitter.com/intent/user?user_id={}".format(tweet['user']['id'])
        message = "<b><u><a href='{}'>@{}</a> ({})</u></b>: {} <i>{}</i>".format(userurl, twhandle, username, text,
                                                                                 timeago)
        try:
        ...
        tweet = re.sub(r"#([a-zA-Z0-9]*)", r"<a href='https://twitter.com/hashtag/\1'>#\1</a>",
                       soup.title.text.split(" on Twitter: ")[1].strip())
        message = "<b><a href='{}'>@{}</a> [{}]</b>: {}".format("https://twitter.com/{}".format(username), username,
                                                                twhandle, tweet)
        yield from bot.coro_send_message(event.conv, message)

...
def setweatherlocation(bot, event, *args):
    ...
    yield from bot.coro_send_message(event.conv_id,
                                     _('This hangouts default location has been set to {}.'.format(location)))
...
def weather(bot, event, *args):
    ...
    if weather:
        ...
        yield from bot.coro_send_message(event.conv_id,
                                         'There was an error retrieving the weather, guess you need to look outside.')
...
def _format_current_weather(weather):
    ...
    if 'temperature' in weather:
        weatherStrings.append(
            "It is currently: <b>{0}°{1}</b>".format(round(weather['temperature'], 2), weather['units']['temperature']))
    if 'summary' in weather:
        weatherStrings.append("<i>{0}</i>".format(weather['summary']))
    if 'feelsLike' in weather:
        weatherStrings.append(
            "Feels Like: {0}°{1}".format(round(weather['feelsLike'], 2), weather['units']['temperature']))
    if 'windspeed' in weather:
        weatherStrings.append(
            "Wind: {0} {1} from {2}".format(round(weather['windspeed'], 2), weather['units']['windSpeed'],

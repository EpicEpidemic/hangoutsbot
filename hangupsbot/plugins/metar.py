...
METAR source: http://aviationweather.gov"""
    ...
    if data is None:
        ...
        yield from bot.coro_send_message(event.conv_id,
                                         "The response did not contain METAR information, check the ICAO airport code and try again.")
    else:
...
TAF source: http://aviationweather.gov"""
    ...
    if data is None:
        ...
        yield from bot.coro_send_message(event.conv_id,
                                         "The response did not contain TAF information, check the ICAO airport code and try again.")
    else:
        yield from bot.coro_send_message(event.conv_id, data[0].text)
